...
export async function listAccountStatementSummaries(
  ...
  const branchClause = ctx.allowedBranchIds.length > 0 ? 'AND i.branch_id IN (:branchIds)' : ''
  ...
  const rows = await sequelize.query<RawSummaryRow>(`
    SELECT
      ...
      COALESCE(SUM(CAST(i.total AS NUMERIC)), 0)::text AS total_invoiced,
      COALESCE(SUM(CAST(i.paid_amount AS NUMERIC)), 0)::text AS total_paid,
      COALESCE(SUM(CAST(i.balance AS NUMERIC)), 0)::text AS balance,
      COALESCE(SUM(
        CASE WHEN i.due_date < NOW() AND CAST(i.balance AS NUMERIC) > 0
    ...
    ORDER BY
      ...
      COALESCE(SUM(CAST(i.balance AS NUMERIC)), 0) DESC,
      c.legal_name ASC
  ...
  const data: AccountStatementSummaryRow[] = rows.map((row) => {
    const balance = new Decimal(String(row.balance ?? '0'))
    const overdue = new Decimal(String(row.overdue_balance ?? '0'))

    let debtStatus: AccountStatementSummaryRow['debt_status'] = 'up_to_date'
