...
// The real `CookieConsentBanner` is gated behind `COOKIE_CONSENT_ENABLED` (currently `false`)
// and renders nothing at runtime — see src/lib/cookie-consent.ts. This story renders the same
// markup directly so the visual design stays covered in Storybook while the feature is off.
...
const meta: Meta<typeof CookieConsentBannerPreview> = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Apagado por el flag `COOKIE_CONSENT_ENABLED` (`src/lib/cookie-consent.ts`). Se activa el día que se sume analítica no esencial.',
      },
    },
  },
  decorators: [
