...
export const GET = withPermission<P>('products:read', async (_req, ctx, session) => {
  ...
  try {
    const priceList = await getPriceList(id, tenantResult.ctx.orgId)
...
export const PATCH = withPermission<P>('products:write', async (req, ctx, session) => {
  ...
  try {
    const priceList = await updatePriceList(id, parsed.data, resolveActorId(session), tenantResult.ctx.orgId)
    return NextResponse.json(priceList)
  } catch (err) {
...
export const DELETE = withPermission<P>('products:delete', async (_req, ctx, session) => {
  ...
  try {
    await deletePriceList(id, resolveActorId(session), tenantResult.ctx.orgId)
