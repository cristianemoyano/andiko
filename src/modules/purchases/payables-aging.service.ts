...
export type PayablesAgingTotals = Omit<
  PayablesAgingRow,
...
const ZERO_TOTALS: PayablesAgingTotals = {
  invoices_count: 0,
  current: '0.00',
  bucket_1_30: '0.00',
  bucket_31_60: '0.00',
...
/** Per-invoice bucket expressions, applied only to open balances (`si.balance > 0`). */
const BUCKET_COLUMNS = `
    COUNT(si.id)::int AS invoices_count,
    COALESCE(SUM(CASE WHEN CAST(si.balance AS NUMERIC) > 0
    ...
    COALESCE(SUM(CASE WHEN CAST(si.balance AS NUMERIC) > 0 AND si.due_date < NOW()
      ...
      THEN CAST(si.balance AS NUMERIC) ELSE 0 END), 0)::text AS bucket_31_60,
    COALESCE(SUM(CASE WHEN CAST(si.balance AS NUMERIC) > 0 AND si.due_date < NOW()
      AND EXTRACT(DAY FROM NOW() - si.due_date) BETWEEN 61 AND 90
      THEN CAST(si.balance AS NUMERIC) ELSE 0 END), 0)::text AS bucket_61_90,
    COALESCE(SUM(CASE WHEN CAST(si.balance AS NUMERIC) > 0 AND si.due_date < NOW()
      AND EXTRACT(DAY FROM NOW() - si.due_date) > 90
      THEN CAST(si.balance AS NUMERIC) ELSE 0 END), 0)::text AS bucket_90_plus,
    COALESCE(SUM(CAST(si.balance AS NUMERIC)), 0)::text AS balance
`

function mapRow(row: RawAgingRow): PayablesAgingRow {
  return {
...
function mapTotals(row: RawTotalsRow | undefined): PayablesAgingTotals {
...
export async function getPayablesAging(query: PayablesAgingQuery, ctx: TenantContext) {
  ...
  const rows = await sequelize.query<RawAgingRow>(`
    SELECT
      ...
      ${BUCKET_COLUMNS}
    ${groupedFromClause}
    ORDER BY COALESCE(SUM(CAST(si.balance AS NUMERIC)), 0) DESC, c.legal_name ASC
  ...
  const [totalsRow] = await sequelize.query<RawTotalsRow>(`
    ...
    FROM supplier_invoices si
    JOIN contacts c ON c.id = si.contact_id AND c.deleted_at IS NULL
    WHERE si.contact_id IN (SELECT id FROM contacts_with_balance)
