...
export function NotasDeCreditoClient() {
  ...
  useEffect(() => {
    ...
    fetchJson<{ data: InvoiceOption[] }>(
      ...
      .then(payload => {
        if (cancelled) return
        const issued = (payload?.data ?? []).filter(i => ['issued', 'partially_paid'].includes(i.status))
        setInvoiceOptions(issued)
      })
  ...
  // When invoice is selected, suggest its balance as net amount
  useEffect(() => {
    if (!invoiceId) return
    const inv = invoiceOptions.find(i => i.id === invoiceId)
    if (inv && new Decimal(inv.balance).gt(0)) {
      // Suggest balance as gross total; back-calculate net for selected IVA rate
      const gross = new Decimal(inv.balance)
      const net = gross.div(new Decimal(1).plus(new Decimal(ivaRate)))
      queueMicrotask(() => setNetAmount(net.toDecimalPlaces(2).toString()))
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setServerError('')
    try {
      await fetchJson('/api/v1/sales/credit-notes', {
        method: 'POST',
        ...
        <form key={formKey} onSubmit={handleSubmit} className="space-y-4 p-5 min-w-[440px]">
          ...
          <FormField label="Motivo *">
            <Input
              ...
              placeholder="Ej: Devolución de mercadería, error de precio…"
            />
          </FormField>
