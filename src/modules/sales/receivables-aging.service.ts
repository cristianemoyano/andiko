...
export type ReceivablesAgingTotals = Omit<
  ReceivablesAgingRow,
...
const ZERO_TOTALS: ReceivablesAgingTotals = {
  invoices_count: 0,
  current: '0.00',
  bucket_1_30: '0.00',
  bucket_31_60: '0.00',
...
/** Per-invoice bucket expressions, applied only to open balances (`i.balance > 0`). */
const BUCKET_COLUMNS = `
    COUNT(i.id)::int AS invoices_count,
    COALESCE(SUM(CASE WHEN CAST(i.balance AS NUMERIC) > 0
    ...
    COALESCE(SUM(CASE WHEN CAST(i.balance AS NUMERIC) > 0 AND i.due_date < NOW()
      ...
      THEN CAST(i.balance AS NUMERIC) ELSE 0 END), 0)::text AS bucket_31_60,
    COALESCE(SUM(CASE WHEN CAST(i.balance AS NUMERIC) > 0 AND i.due_date < NOW()
      AND EXTRACT(DAY FROM NOW() - i.due_date) BETWEEN 61 AND 90
      THEN CAST(i.balance AS NUMERIC) ELSE 0 END), 0)::text AS bucket_61_90,
    COALESCE(SUM(CASE WHEN CAST(i.balance AS NUMERIC) > 0 AND i.due_date < NOW()
      AND EXTRACT(DAY FROM NOW() - i.due_date) > 90
      THEN CAST(i.balance AS NUMERIC) ELSE 0 END), 0)::text AS bucket_90_plus,
    COALESCE(SUM(CAST(i.balance AS NUMERIC)), 0)::text AS balance
`

function mapRow(row: RawAgingRow): ReceivablesAgingRow {
  return {
...
function mapTotals(row: RawTotalsRow | undefined): ReceivablesAgingTotals {
...
export async function getReceivablesAging(query: ReceivablesAgingQuery, ctx: TenantContext) {
  ...
  const rows = await sequelize.query<RawAgingRow>(`
    SELECT
      ...
      ${BUCKET_COLUMNS}
    ${groupedFromClause}
    ORDER BY COALESCE(SUM(CAST(i.balance AS NUMERIC)), 0) DESC, c.legal_name ASC
  ...
  // restrict to contacts with balance > 0 via a CTE, then aggregate their invoices in one shot.
  const [totalsRow] = await sequelize.query<RawTotalsRow>(`
    ...
    FROM invoices i
    JOIN contacts c ON c.id = i.contact_id AND c.deleted_at IS NULL
    WHERE i.contact_id IN (SELECT id FROM contacts_with_balance)
