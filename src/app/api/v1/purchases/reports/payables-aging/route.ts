...
import { payablesAgingQuerySchema } from '@/modules/purchases/supplier-account-statement-summary.schema'
import { getPayablesAging } from '@/modules/purchases/payables-aging.service'

export const GET = withPermission('accounting:read', async (req, _ctx, session) => {
  const parsed = payablesAgingQuerySchema.safeParse(Object.fromEntries(req.nextUrl.searchParams))
  if (!parsed.success) {
  ...
  try {
    ...
    const result = await getPayablesAging(parsed.data, tenantCtx)
