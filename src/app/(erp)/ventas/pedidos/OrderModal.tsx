...
import type { Order, IvaRate, PaymentCondition } from '../types'
...
export function OrderModal({ open, order, onClose, onSaved }: OrderModalProps) {
  ...
  const [contactId, setContactId]               = useState<string | null>(order?.contact_id ?? null)
  const [branchId, setBranchId]                 = useState<string | null>(order?.branch_id ?? null)
  const [requiredDate, setRequiredDate]         = useState<Date | null>(
    order?.required_date ? new Date(order.required_date) : null
  )
  const [paymentCondition, setPaymentCondition] = useState<PaymentCondition>(
    order?.payment_condition ?? 'cash'
  )
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const url    = isEdit ? `/api/v1/sales/orders/${order.id}` : '/api/v1/sales/orders'
  ...
  async function handleDelete() {
    if (!order) return
    await fetch(`/api/v1/sales/orders/${order.id}`, { method: 'DELETE' })
  ...
  return (
    <>
      <Dialog
        ...
        title={isEdit ? `Editar ${order.order_number}` : 'Nuevo pedido'}
        size="xl"
      >
        <form key={`${order?.id ?? 'new'}-${String(open)}`} onSubmit={handleSubmit} noValidate>
          <div className="flex flex-col gap-4">
            ...
            {/* Items table */}
            <div>
              <p className="text-[11px] font-semibold text-zinc-500 uppercase tracking-wide mb-2">Ítems</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField label="Notas para el cliente" htmlFor="notes">
                <Textarea id="notes" name="notes" defaultValue={order?.notes ?? ''} rows={2} placeholder="Condiciones, aclaraciones…" />
              </FormField>
              <FormField label="Notas internas" htmlFor="internal_notes">
                <Textarea id="internal_notes" name="internal_notes" defaultValue={order?.internal_notes ?? ''} rows={2} placeholder="Visible solo para el equipo…" />
              </FormField>
            </div>
      ...
      <ConfirmDialog
        ...
        description={`¿Eliminar ${order?.order_number}? Esta acción no se puede deshacer.`}
