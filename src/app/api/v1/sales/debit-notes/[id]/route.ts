...
import { updateDebitNoteSchema } from '@/modules/sales/debit-note.schema'
import { getDebitNote, updateDebitNote, deleteDebitNote } from '@/modules/sales/debit-notes.service'
...
export const GET = withPermission<P>('sales:read', async (_req, ctx, session) => {
  ...
  try {
    ...
    const note = await getDebitNote(id, tenantCtx)
...
export const DELETE = withPermission<P>('sales:delete', async (_req, ctx, session) => {
  ...
  try {
    const tenantCtx = await makeTenantContext(session.user)
    await deleteDebitNote(id, tenantCtx)
