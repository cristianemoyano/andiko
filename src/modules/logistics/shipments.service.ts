...
import { assertShipmentTransition, canTransitionShipment, TERMINAL_SHIPMENT_STATUSES, type ShipmentEventSource, type ShipmentStatus } from './logistics.constants'
import { getFulfillmentProvider, trackingUrlFor } from './providers'
import type {
  ShipmentInput, ShipmentDispatchInput, ShipmentEventInput,
  ShipmentDeliverInput, ShipmentFailInput, ShipmentAssignDriverInput, ShipmentQuery,
} from './shipment.schema'

// Estados del pedido en los que se puede generar un envío.
...
export async function listShipments(query: ShipmentQuery, ctx: TenantContext) {
  ...
  const { rows, count } = await Shipment.findAndCountAll({
    ...
    attributes: [
      ...
      'provider_kind', 'tracking_number', 'tracking_url', 'assigned_driver_id', 'vehicle_ref',
...
export async function createShipmentForOrder(
  ...
  const run = async (t: Transaction) => {
    ...
    if (!SHIPPABLE_ORDER_STATUSES.includes(order.status)) throw new Error('ORDER_NOT_SHIPPABLE')
    ...
    const orderItems = (order.get('items') as SalesOrderItem[] | undefined) ?? []
    const byId = new Map(orderItems.map(item => [item.id, item]))

    // Sin items explícitos: enviar todo lo pendiente del pedido.
    ...
    const trackingNumber = input.tracking_number ?? null
    ...
    const shipment = await Shipment.create(
      {
        ...
        vehicle_ref:        input.vehicle_ref ?? null,
    ...
    await shipment.update(
      {
        ...
        vehicle_ref:        input.vehicle_ref ?? shipment.vehicle_ref,
        updated_by:         actorId,
      },
      { transaction: t },
    )
...
export async function assignShipmentDriver(id: string, input: ShipmentAssignDriverInput, ctx: TenantContext, actorId: string) {
  const shipment = await Shipment.findOne({ where: { id, org_id: ctx.orgId } })
  if (!shipment) throw new Error('SHIPMENT_NOT_FOUND')
  void whereBranch(ctx, shipment.branch_id)
  if (shipment.provider_kind !== 'in_house') throw new Error('DRIVER_ONLY_FOR_IN_HOUSE')
  if (TERMINAL_SHIPMENT_STATUSES.includes(shipment.status)) throw new Error('SHIPMENT_ALREADY_CLOSED')
  ...
  logger.info({ shipmentId: id, driverId: input.assigned_driver_id, orgId: ctx.orgId, actorId }, 'shipment driver assigned')
...
async function syncOrderDeliveredState(orderId: string, deliveredAt: Date, actorId: string, t: Transaction) {
  ...
  const items = (order.get('items') as SalesOrderItem[] | undefined) ?? []
  const fullyShipped = items.length > 0 && items.every(
