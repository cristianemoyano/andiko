      ...
      {created ? (
        <div className="p-6 flex flex-col gap-4 w-full min-w-0 overflow-hidden">
          <div className="flex items-start gap-3 p-3 bg-green-50 border border-green-200 rounded-md">
            ...
            <div>
              <p className="text-sm font-medium text-green-800">Dispositivo registrado</p>
              <p className="text-xs text-green-700 mt-0.5">Copiá el token y configuralo en el POS. No se puede ver de nuevo.</p>
            </div>
          </div>

          <div>
            <p className="text-xs font-medium text-zinc-500 mb-1">Device ID</p>
            <p className="text-sm font-mono text-zinc-800">{created.device_id}</p>
          </div>

          <div>
            <p className="text-xs font-medium text-zinc-500 mb-1">API Token</p>
            <div className="flex items-start gap-2 min-w-0">
              <code className="flex-1 min-w-0 block text-xs font-mono bg-zinc-100 border border-zinc-200 rounded px-3 py-2 text-zinc-800 overflow-x-auto whitespace-nowrap">
                {created.api_token}
              </code>
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="device_id" className="text-xs font-medium text-zinc-700">Device ID</label>
            <input
              id="device_id"
              value={deviceId}
              onChange={e => setDeviceId(e.target.value)}
              placeholder="auto-generado desde el nombre"
              className="border border-zinc-300 rounded-md px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-brand-500 text-zinc-700"
            />
            {errors.device_id && <p className="text-xs text-red-600">{errors.device_id}</p>}
            <p className="text-xs text-zinc-400">Se genera automáticamente al escribir el nombre. Podés editarlo.</p>
          </div>
