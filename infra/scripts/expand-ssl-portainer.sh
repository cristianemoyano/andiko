#!/usr/bin/env bash
# Expand the existing Let's Encrypt cert to include portainer.${DOMAIN}.
# Run once on a VPS that already has HTTPS for andiko.cloud without the Portainer SAN.
set -euo pipefail
...
echo "Expanding certificate to include ${PORTAINER_DOMAIN} ..."
docker run --rm \
...
echo "Done. Verify: curl -sf -u USER:PASS https://${PORTAINER_DOMAIN}/api/status"
