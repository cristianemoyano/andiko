...
import { getEffectivePricesForVariants } from '@/modules/catalog/price-list.service'
...
export const GET = withPermission('sales:read', async (req, _ctx, session) => {
  ...
  try {
    ...
    const prices = await getEffectivePricesForVariants(parsed.data.price_list_id, parsed.data.variant_ids, ctx.orgId)
