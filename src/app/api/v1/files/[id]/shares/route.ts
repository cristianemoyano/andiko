import { NextResponse } from 'next/server'
import { withTenantAuth } from '@/lib/api-handler'
import { shareSchema } from '@/modules/storage/storage.schema'
import { shareFile, getFile } from '@/modules/storage/storage.service'
import { buildFileActor, storageErrorResponse } from '@/modules/storage/storage.http'

type P = { id: string }

export const GET = withTenantAuth<P>(async (_req, ctx, session, tenant) => {
  const { id } = await ctx.params
  try {
    const { shares } = await getFile(id, buildFileActor(session, tenant))
    return NextResponse.json({ data: shares })
  } catch (err) {
    const resp = storageErrorResponse(err)
    if (resp) return resp
    throw err
  }
})

export const POST = withTenantAuth<P>(async (req, ctx, session, tenant) => {
  const { id } = await ctx.params
  const body = await req.json()
  const parsed = shareSchema.safeParse(body)
  if (!parsed.success) {
    return NextResponse.json(
      { error: 'Invalid input', code: 'VALIDATION_ERROR', details: parsed.error.flatten() },
      { status: 422 },
    )
  }
  try {
    const share = await shareFile(id, parsed.data, buildFileActor(session, tenant))
    return NextResponse.json(share, { status: 201 })
  } catch (err) {
    const resp = storageErrorResponse(err)
    if (resp) return resp
    throw err
  }
})
