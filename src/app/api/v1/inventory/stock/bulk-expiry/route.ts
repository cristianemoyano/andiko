...
import { bulkStockExpirySchema } from '@/modules/inventory/stock-level.schema'
import { bulkSetStockExpiry } from '@/modules/inventory/stock-items.service'

export const POST = withPermission('inventory:write', async (req, _ctx, session) => {
  const body = await req.json()
  const parsed = bulkStockExpirySchema.safeParse(body)
  if (!parsed.success) {
  ...
  try {
    ...
    const result = await bulkSetStockExpiry(ctx, parsed.data)
