...
vi.mock('@/modules/sales/payment.model', () => ({ default: { findByPk: paymentFindByPk } }))

import { postSalesPaymentAccounting } from './sales-payment-accounting.service'
...
describe('postSalesPaymentAccounting', () => {
  it('is idempotent — no-ops when an entry already exists', async () => {
    ...
    await postSalesPaymentAccounting('pay-1', ctx, t)
  ...
  it('debits Banco for non-cash payment methods and credits receivable', async () => {
    paymentFindByPk.mockResolvedValue(mockPayment({ payment_method: 'transfer' }))

    await postSalesPaymentAccounting('pay-1', ctx, t)

    expect(entryCreate).toHaveBeenCalledWith(
      expect.objectContaining({ source_type: 'sales_payment', source_id: 'pay-1', status: 'posted', total_debit: '100.00', total_credit: '100.00' }),
      expect.anything(),
    )
    const lines = lineBulkCreate.mock.calls[0]![0] as Array<{ account_id: string; debit: string; credit: string }>
    expect(lines.find(l => l.account_id === 'acc-bank')).toMatchObject({ debit: '100.00', credit: '0.00' })
    expect(lines.find(l => l.account_id === 'acc-receivable')).toMatchObject({ debit: '0.00', credit: '100.00' })
  })

  it('debits Caja for cash payments', async () => {
    paymentFindByPk.mockResolvedValue(mockPayment({ payment_method: 'cash' }))

    await postSalesPaymentAccounting('pay-1', ctx, t)

    const lines = lineBulkCreate.mock.calls[0]![0] as Array<{ account_id: string; debit: string; credit: string }>
    expect(lines.find(l => l.account_id === 'acc-cash')).toMatchObject({ debit: '100.00', credit: '0.00' })
  })
})
