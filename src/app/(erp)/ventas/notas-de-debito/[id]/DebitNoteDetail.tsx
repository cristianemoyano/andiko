...
export function DebitNoteDetail() {
  ...
  const debitNoteSecondaryActions: PageAction[] = editing
    ? []
    : [
        { id: 'print', label: 'Imprimir', href: `/ventas/notas-de-debito/${id}/print`, openInNewTab: true },
  ...
  const displayNumber = resolveSalesDocumentDisplay({
    internalNumber: note.debit_note_number,
      ...
      <TopBar
        ...
        actions={
          <PageActionBar
            ...
            secondary={debitNoteSecondaryActions}
          />
        }
      />
      <VentasSubNav />
                ...
                {loadingInvoices ? (
                  ...
                  >
                    ...
                    {invoiceOptions.map(inv => (
                      <option key={inv.id} value={inv.id}>{invoiceOptionLabel(inv)}</option>
                    ))}
                  </select>
                )}
                {selectedInvoice && (
