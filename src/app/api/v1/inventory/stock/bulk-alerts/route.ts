...
import { bulkStockMinimumSchema } from '@/modules/inventory/stock-level.schema'
import { bulkSetStockMinimum } from '@/modules/inventory/stock-items.service'

export const POST = withPermission('inventory:write', async (req, _ctx, session) => {
  const body = await req.json()
  const parsed = bulkStockMinimumSchema.safeParse(body)
  if (!parsed.success) {
  ...
  try {
    ...
    const result = await bulkSetStockMinimum(ctx, parsed.data)
