...
import { getAccountStatement } from '@/modules/sales/account-statement.service'

type P = { id: string }

export const GET = withPermission<P>('sales:read', async (req, ctx, session) => {
  ...
  try {
    ...
    const statement = await getAccountStatement(id, parsed.data, tenantCtx)
    ...
    if (err instanceof Error && err.message === 'CONTACT_NOT_FOUND') {
      return NextResponse.json({ error: 'Cliente no encontrado', code: 'CONTACT_NOT_FOUND' }, { status: 404 })
    }
