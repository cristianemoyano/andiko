...
import { previewCustomerImport } from '@/modules/integrations/woocommerce/woo-customers.service'
import { woocommerceCustomerImportPreviewSchema } from '@/modules/integrations/woocommerce/woocommerce.schema'

export const POST = withPermission<{ id: string }>('settings:write', async (req, ctx, session) => {
  const { id } = await ctx.params
  const body = woocommerceCustomerImportPreviewSchema.parse(await req.json().catch(() => ({})))
  try {
    const ctxTenant = await makeTenantContext(session.user)
    const site = await getSite(id, ctxTenant.orgId)
    return NextResponse.json(await previewCustomerImport(site, body))
  } catch (err: unknown) {
