...
describe('createPayment', () => {
  ...
  it('creates payment for an issued invoice', async () => {
    ...
    const result = await createPayment(input, 'org-1', 'actor-1')
  ...
  it('creates payment for a partially_paid invoice', async () => {
    ;(Invoice.findByPk as Mock).mockResolvedValue(mockInvoice('partially_paid'))
    ;(Payment.create as Mock).mockResolvedValue({ id: 'pay-2', ...input })

    await expect(createPayment(input, 'org-1', 'actor-1')).resolves.toBeDefined()
  })

  it('throws INVOICE_NOT_FOUND when invoice is missing', async () => {
    ;(Invoice.findByPk as Mock).mockResolvedValue(null)
    await expect(createPayment(input, 'org-1', 'actor-1')).rejects.toThrow('INVOICE_NOT_FOUND')
  })

  it('throws INVOICE_CANCELLED for cancelled invoices', async () => {
    ;(Invoice.findByPk as Mock).mockResolvedValue(mockInvoice('cancelled'))
    await expect(createPayment(input, 'org-1', 'actor-1')).rejects.toThrow('INVOICE_CANCELLED')
  })

  it('throws INVOICE_NOT_ISSUED for draft invoices', async () => {
    ;(Invoice.findByPk as Mock).mockResolvedValue(mockInvoice('draft'))
    await expect(createPayment(input, 'org-1', 'actor-1')).rejects.toThrow('INVOICE_NOT_ISSUED')
  })
