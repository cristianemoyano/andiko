...
import { getPayablesAging } from './payables-aging.service'
...
describe('getPayablesAging', () => {
  it('maps rows with normalized money strings per bucket', async () => {
    ...
    const result = await getPayablesAging({ page: 1, limit: 20 }, tenantCtx)
  ...
  it('filters by an explicit branch_id when provided', async () => {
    ...
    await getPayablesAging({ page: 1, limit: 20, branch_id: 'branch-1' }, tenantCtx)

    const [sql, options] = queryMock.mock.calls[0] as [string, { replacements: Record<string, unknown> }]
    expect(sql).toContain('si.branch_id = :branchId')
  ...
  it('searches by name or CUIT with a bound ILIKE pattern', async () => {
    ...
    await getPayablesAging({ page: 1, limit: 20, search: 'acme' }, tenantCtx)
  ...
  it('paginates with bound limit/offset and a separate count query', async () => {
    ...
    const result = await getPayablesAging({ page: 3, limit: 10 }, tenantCtx)
