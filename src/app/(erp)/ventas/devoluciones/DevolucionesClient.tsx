...
export function DevolucionesClient() {
    ...
    void (async () => {
      ...
      try {
        const data = await fetchJson<{ data: ReturnRow[]; total: number }>(
          `/api/v1/sales/returns?page=${page}&limit=${PAGE_SIZE}`,
        )
        if (!mounted) return
        setRows(data.data ?? [])
  ...
  const columns: Column<ReturnRow>[] = [
    ...
    { key: 'order', header: 'Pedido', render: r => r.order?.order_number ?? '—' },
    {
      key: 'status',
      header: 'Estado',
    ...
    { key: 'difference_total', header: 'Diferencia', render: r => formatARS(r.difference_total), align: 'right' },
  ]

  return (
    <div className="flex flex-col h-full">
      <TopBar breadcrumbs={[{ label: 'Ventas', href: '/ventas/presupuestos' }, { label: 'Devoluciones' }]} />
      <VentasSubNav />
      <PageBody>
        {listError && (
        ...
        <DataTable<ReturnRow>
          ...
          data={rows}
          keyExtractor={r => r.id}
          onRowClick={r => router.push(`/ventas/devoluciones/${r.id}`)}
