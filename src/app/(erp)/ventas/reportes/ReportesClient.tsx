  ...
  const columns = useMemo<Column<ReportRow>[]>(() => {
    const cols: Column<ReportRow>[] = [
      {
        ...
        render: row => (
          <div className="min-w-0">
            ...
            {row.secondary_label ? (
              <p className="truncate text-[12px] text-zinc-500">{row.secondary_label}</p>
            ) : null}
          </div>
        ),
      },
    ...
    if (groupBy === 'product') {
      cols.push({
        ...
        align: 'right',
        render: row => (
          <span className="tabular-nums text-zinc-700">{row.quantity ?? '—'}</span>
        ),
      })
    }
    cols.push(
      {
        ...
        header: 'Comprobantes',
        align: 'right',
        render: row => <span className="tabular-nums text-zinc-700">{row.documents}</span>,
      },
      {
        key: 'subtotal',
        header: 'Subtotal',
      ...
      },
      {
        ...
        header: 'IVA',
        align: 'right',
        render: row => <span className="tabular-nums text-zinc-700">{formatARS(row.tax)}</span>,
      },
      {
        key: 'total',
        header: 'Total',
      ...
      <div className="flex-1 overflow-auto p-5">
        ...
        {groupBy === 'period' && rows.length > 0 && (
          <div className="mb-4 rounded border border-zinc-200 bg-white p-4">
            <p className="mb-2 text-[12px] uppercase tracking-wide text-zinc-500">Ventas por período</p>
