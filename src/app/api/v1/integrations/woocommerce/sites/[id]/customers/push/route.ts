...
import { pushCustomersForSite } from '@/modules/integrations/woocommerce/woo-customers.service'
...
export const POST = withPermission<{ id: string }>('settings:write', async (_req, ctx, session) => {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, ...(await pushCustomersForSite(site)) })
  } catch (err: unknown) {
