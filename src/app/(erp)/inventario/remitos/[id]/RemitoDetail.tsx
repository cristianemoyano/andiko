    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-5 overflow-auto">
        <div className="max-w-4xl mx-auto flex flex-col gap-5">
          ...
          {(isIssued || isDelivered) && !note.deducts_stock && (
            <div className="px-4 py-3 bg-amber-50 border border-amber-200 rounded-sm text-sm text-amber-700">
          ...
          <div className="bg-white border border-zinc-200 rounded-sm px-5 py-4 flex items-start justify-between gap-4">
            <div>
              <p className="text-[11px] text-zinc-400 font-semibold uppercase tracking-wide mb-1">Remito de entrega</p>
              <h1 className="text-[20px] font-bold text-zinc-900 tracking-tight">{note.delivery_number}</h1>
          ...
          <div className="bg-white border border-zinc-200 rounded-sm p-5">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-[13px]">
              <div>
                ...
                <p className="text-zinc-800">{note.branch?.name ?? '—'}</p>
              </div>
              <div>
                <p className="text-[11px] text-zinc-400 font-medium uppercase tracking-wide mb-0.5">Depósito</p>
                <p className="text-zinc-800">{note.warehouse?.name ?? '—'}</p>
              </div>
              <div>
                <p className="text-[11px] text-zinc-400 font-medium uppercase tracking-wide mb-0.5">Fecha de entrega</p>
                <p className="text-zinc-800">
                  {note.delivery_date ? new Date(note.delivery_date).toLocaleDateString('es-AR') : '—'}
                </p>
              </div>
              <div>
                <p className="text-[11px] text-zinc-400 font-medium uppercase tracking-wide mb-0.5">Creado</p>
                <p className="text-zinc-800">{new Date(note.created_at).toLocaleDateString('es-AR')}</p>
              </div>
              {note.carrier && (
                <div>
                  <p className="text-[11px] text-zinc-400 font-medium uppercase tracking-wide mb-0.5">Transportista</p>
                  <p className="text-zinc-800">{note.carrier}</p>
                </div>
              )}
              {note.tracking_code && (
                <div>
                  <p className="text-[11px] text-zinc-400 font-medium uppercase tracking-wide mb-0.5">Seguimiento</p>
                  <p className="text-zinc-800">{note.tracking_code}</p>
                </div>
              )}
              {note.issuer && (
                <div>
                  <p className="text-[11px] text-zinc-400 font-medium uppercase tracking-wide mb-0.5">Emitido por</p>
                  <p className="text-zinc-800">{note.issuer.name}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-zinc-200 rounded-sm overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-zinc-100">
                {(note.items ?? []).length === 0 ? (
                  <tr>
                    <td colSpan={2} className="px-4 py-8 text-center text-zinc-400 text-sm">Sin ítems</td>
                  </tr>
                ) : (
                  (note.items ?? []).map(item => (
          ...
          {note.order && (
            <div className="bg-white border border-zinc-200 rounded-sm px-5 py-4">
              <p className="text-[11px] text-zinc-400 font-semibold uppercase tracking-wide mb-2">Pedido de venta vinculado</p>
              <div
              ...
              >
                <span className="text-[13px] font-medium text-zinc-900">{note.order.order_number}</span>
