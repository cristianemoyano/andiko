...
import { getExpensesPayablesAging } from './expenses-payables-aging.service'
...
describe('getExpensesPayablesAging', () => {
  it('maps rows with normalized money strings per bucket', async () => {
    mockCalls([baseRow])

    const result = await getExpensesPayablesAging({ page: 1, limit: 20 }, tenantCtx)
  ...
  it('scopes by org, open payable statuses only, and always applies HAVING balance > 0', async () => {
    ...
    await getExpensesPayablesAging({ page: 1, limit: 20 }, tenantCtx)

    const [sql, options] = queryMock.mock.calls[0] as [string, { replacements: Record<string, unknown> }]
    expect(sql).toContain('e.org_id = :orgId')
    expect(sql).toContain('e.status IN (:openStatuses)')
    expect(sql).toContain('e.deleted_at IS NULL')
    expect(sql).toContain('HAVING COALESCE(SUM(CAST(e.balance AS NUMERIC)), 0) > 0')
    expect(options.replacements.orgId).toBe('org-1')
    expect(options.replacements.openStatuses).toEqual(['received', 'partially_paid'])
  })

  it('excludes expenses without an assigned supplier (contact_id IS NOT NULL)', async () => {
    mockCalls([])

    await getExpensesPayablesAging({ page: 1, limit: 20 }, tenantCtx)

    const [sql] = queryMock.mock.calls[0] as [string]
    expect(sql).toContain('e.contact_id IS NOT NULL')
  })
  ...
  it('computes bucket ranges via EXTRACT(DAY FROM NOW() - due_date)', async () => {
    ...
    expect(sql).toContain('EXTRACT(DAY FROM NOW() - e.due_date) > 90')
  })

  it('filters by allowed branches when the context restricts them', async () => {
    mockCalls([])

    await getExpensesPayablesAging(
      { page: 1, limit: 20 },
      { ...tenantCtx, allowedBranchIds: ['branch-1', 'branch-2'] },
    )

    const [sql, options] = queryMock.mock.calls[0] as [string, { replacements: Record<string, unknown> }]
    expect(sql).toContain('e.branch_id IN (:branchIds)')
  ...
  it('filters by an explicit branch_id when provided', async () => {
    ...
    await getExpensesPayablesAging({ page: 1, limit: 20, branch_id: 'branch-1' }, tenantCtx)

    const [sql, options] = queryMock.mock.calls[0] as [string, { replacements: Record<string, unknown> }]
    expect(sql).toContain('e.branch_id = :branchId')
  ...
  it('searches by name or CUIT with a bound ILIKE pattern', async () => {
    ...
    await getExpensesPayablesAging({ page: 1, limit: 20, search: 'acme' }, tenantCtx)
  ...
  it('paginates with bound limit/offset and a separate count query', async () => {
    ...
    const result = await getExpensesPayablesAging({ page: 3, limit: 10 }, tenantCtx)
