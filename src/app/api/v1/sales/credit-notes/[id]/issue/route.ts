...
import { issueCreditNote } from '@/modules/sales/credit-notes.service'
...
export const POST = withPermission<P>('sales:write', async (_req, ctx, session) => {
  ...
  try {
    ...
    const note = await issueCreditNote(id, tenantCtx)
    ...
    if (err instanceof Error) {
      ...
      if (err.message === 'CREDIT_NOTE_ALREADY_ISSUED') return NextResponse.json({ error: 'La nota de crédito ya fue emitida', code: err.message }, { status: 409 })
    }
    throw err
  }
})
