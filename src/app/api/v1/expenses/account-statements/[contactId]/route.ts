...
import { getExpenseSupplierAccountStatement } from '@/modules/expenses/expense-supplier-statement.service'

type P = { contactId: string }

export const GET = withPermission<P>('expenses:read', async (req, ctx, session) => {
  ...
  try {
    ...
    const statement = await getExpenseSupplierAccountStatement(contactId, parsed.data, tenantCtx)
