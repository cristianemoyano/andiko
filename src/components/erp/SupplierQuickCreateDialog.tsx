...
export function SupplierQuickCreateDialog({
  ...
  return (
    <Dialog
      ...
      description="Creá un proveedor sin salir del flujo actual."
      size="sm"
    >
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <FormField label="Razón social" htmlFor="supplier_quick_legal_name" required error={errors.legal_name?.[0]}>
          <Input
            id="supplier_quick_legal_name"
            value={legalName}
        ...
        <FormField label="Condición IVA" htmlFor="supplier_quick_iva" required error={errors.iva_condition?.[0]}>
          <select
            id="supplier_quick_iva"
            value={ivaCondition}
