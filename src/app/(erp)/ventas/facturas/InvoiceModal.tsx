...
import type { Invoice, IvaRate, PaymentCondition } from '../types'
...
export function InvoiceModal({ open, invoice, onClose, onSaved }: InvoiceModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const url    = isEdit ? `/api/v1/sales/invoices/${invoice.id}` : '/api/v1/sales/invoices'
  ...
  async function handleDelete() {
    if (!invoice) return
    await fetch(`/api/v1/sales/invoices/${invoice.id}`, { method: 'DELETE' })
  ...
  return (
    <>
      <Dialog
        ...
        title={isEdit ? `Editar ${invoice.invoice_number}` : 'Nueva factura'}
        size="xl"
      >
        <form key={`${invoice?.id ?? 'new'}-${String(open)}`} onSubmit={handleSubmit} noValidate>
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField label="Fecha de emisión" htmlFor="issue_date">
                <DateInput
                  id="issue_date"
                  value={issueDate}
              ...
              <FormField label="Fecha de vencimiento" htmlFor="due_date">
                <DateInput
                  id="due_date"
                  value={dueDate}
                  onChange={setDueDate}
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField label="Notas para el cliente" htmlFor="notes">
                <Textarea id="notes" name="notes" defaultValue={invoice?.notes ?? ''} rows={2} placeholder="Condiciones, aclaraciones…" />
              </FormField>
              <FormField label="Notas internas" htmlFor="internal_notes">
                <Textarea id="internal_notes" name="internal_notes" defaultValue={invoice?.internal_notes ?? ''} rows={2} placeholder="Visible solo para el equipo…" />
              </FormField>
            </div>
      ...
      <ConfirmDialog
        ...
        description={`¿Eliminar ${invoice?.invoice_number}? Esta acción no se puede deshacer.`}
